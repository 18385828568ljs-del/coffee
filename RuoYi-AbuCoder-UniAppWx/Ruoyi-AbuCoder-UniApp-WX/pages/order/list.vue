		...
		<scroll-view class="order-list" scroll-y @scrolltolower="loadMore">
			...
			>
				<view class="order-card-header">
					...
					<text class="order-status" :style="{ color: getDisplayStatusColor(order) }">
						{{ getDisplayStatusText(order) }}
					</text>
				</view>
				<view v-if="getRefundStatus(order) > 0" class="refund-strip" :class="'status-' + getRefundStatus(order)">
					<text>{{ getRefundStatusText(order) }}</text>
					<text v-if="order.refundReason" class="refund-reason">{{ order.refundReason }}</text>
				</view>
				...
				<view class="order-footer">
					...
					<view class="order-actions">
						<view
							v-if="order.status === 0"
							class="action-btn"
							:class="{ 'action-btn-disabled': isOrderPending(getOrderId(order)) }"
							@tap.stop="cancelOrder(getOrderId(order))"
						>
							<text>{{ isOrderActionPending(getOrderId(order), 'cancel') ? '取消中...' : '取消订单' }}</text>
						</view>
						<view
							v-if="order.status === 0"
						...
						>
							<text>{{ isOrderActionPending(getOrderId(order), 'pay') ? '支付中...' : '立即支付' }}</text>
						</view>
						<view
							v-if="orderType === 'mall' && order.status === 2"
							class="action-btn action-btn-light"
							:class="{ 'action-btn-disabled': isOrderPending(getOrderId(order)) }"
							@tap.stop="confirmOrder(getOrderId(order))"
						>
							<text>{{ isOrderActionPending(getOrderId(order), 'confirm') ? '确认中...' : '确认收货' }}</text>
						</view>
						<view
							v-if="orderType === 'scan' && canUrgeOrder(order)"
							class="action-btn action-btn-light"
							:class="{ 'action-btn-disabled': isOrderPending(getOrderId(order)) }"
							@tap.stop="urgeOrder(getOrderId(order))"
						>
							<text>{{ isOrderActionPending(getOrderId(order), 'urge') ? '催单中...' : '催单' }}</text>
						</view>
					</view>
				</view>
			</navigator>
...
export default {
	...
	methods: {
		...
		getOrderId(order) {
			...
			return order.orderId || order.id || order.order_id || order.orderID || null
		},
		...
		getDetailUrl(order) {
			...
			if (this.orderType === 'scan') {
				return `/pages/scan/detail?orderId=${orderId}`
			}
		...
		canUrgeOrder(order) {
			...
			return (order.status === 2 || order.status === 3)
				&& this.getRefundStatus(order) !== 1
				&& this.getRefundStatus(order) !== 2
		},

		getRefundStatus(order) {
			return Number((order && order.refundStatus) || 0)
		},
		...
		getRefundStatusText(order) {
			...
			return map[this.getRefundStatus(order)] || ''
		},

		refundOverridesOrderStatus(order) {
			const status = this.getRefundStatus(order)
			return status === 1 || status === 2
		},

		getDisplayStatusText(order) {
			if (this.refundOverridesOrderStatus(order)) {
				return this.getRefundStatusText(order)
			}
			return this.getStatusText(order.status)
		},

		getDisplayStatusColor(order) {
			const refundStatus = this.getRefundStatus(order)
			if (refundStatus === 1) return '#B77945'
			if (refundStatus === 2) return '#7C8F81'
			return this.getStatusColor(order.status)
		},
		...
		refreshFirstPageSilently() {
			if (this.loading || this.pendingOrderActionKey || this.pageNum !== 1) {
				return
			}
			this.loadOrders({ refresh: true, silent: true })
		},

		loadOrders(options = {}) {
			if (this.orderType === 'scan') {
				this.loadScanOrders(options)
				return
			}
			if (this.loading || (!this.hasMore && !options.refresh)) {
				return
			}

			this.loading = true
			const pageNum = options.refresh ? 1 : this.pageNum
			const status = this.tabs[this.currentTab].status
			const requestData = status !== null && status !== undefined
				? { pageNum, pageSize: this.pageSize, status }
			...
			requestPromise({
				...
				.then((res) => {
					if (isSuccessResponse(res)) {
						...
						this.orderList = options.refresh ? orders : [...this.orderList, ...orders]
		...
		async payOrder(order) {
			...
			const payType = this.orderType === 'scan' ? 'balance' : ''
			const confirmed = await showConfirm({
				title: '确认支付',
...
.refund-strip.status-2 {
	background: rgba(124, 143, 129, 0.12);
}

.refund-strip.status-2 text {
	color: $accent-success;
}

.refund-strip.status-3 {
	background: rgba(200, 110, 96, 0.12);
}

.refund-strip.status-3 text {
	color: $accent-danger;
}
