...
export default {
	...
	computed: {
		...
		showActionBar() {
			return !this.refundBlocksOrderActions
				&& (this.orderInfo.status === 0 || this.orderInfo.status === 2 || this.canApplyRefund)
		},
	...
	methods: {
		...
		async payOrder() {
			...
			const confirmed = await showConfirm({
				title: '确认支付',
				content: `当前订单需支付 ¥${this.formatMoney(this.orderInfo.payAmount || this.orderInfo.totalAmount)}，确认继续支付吗？`
			})
			if (!confirmed) {
				return
			}
		...
		async confirmRefund(reason) {
			...
			try {
				...
				showError((res.data && res.data.msg) || '退款申请失败')
			} catch (error) {
				showError('退款申请失败')
			} finally {
				this.actionPending = ''
