<template>
	<view class="theme-tab-bar" :class="rootClass" data-skin-component="tabBar" :style="tabBarStyle">
...
export default {
	name: 'ThemeTabBar',
	props: { items: { type: Array, default: () => [] }, current: { type: String, default: '' }, readonly: { type: Boolean, default: false }, embedded: { type: Boolean, default: false } },
	computed: {
		rootClass() { return [`theme-tab-bar-${this.variant}`, { 'theme-tab-bar-embedded': this.embedded }] },
	...
	methods: {
		...
		itemStyle(item) {
			...
			return {
				color: token.color || (active ? slot.activeTextColor : slot.textColor) || (active ? 'var(--theme-primary)' : 'var(--theme-text-secondary)'),
...
<style scoped>
.theme-tab-bar { width: 100%; min-height: 104rpx; padding: 10rpx 20rpx; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); box-sizing: border-box; border-top: 2rpx solid var(--theme-border); background-color: var(--theme-surface); background-repeat: no-repeat; background-position: center; background-size: 100% 100%; }
.theme-tab-bar-embedded { height: 100%; min-height: 0; padding: 0 20rpx; }
