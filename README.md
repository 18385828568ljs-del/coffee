...
- **用户画像推荐** - 根据全部有效历史浏览、首次加购和有效购买生成归一化偏好标签，自动调整商城与点单商品顺序
...
- 从有效订单中提取购买证据
- 按浏览 1、首次加购 4、有效购买 8 累计真实标签证据，并在同一维度内归一化为 0～1 的偏好度
- 价格作为统一画像标签，按 10 元区间生成价格偏好
- 商城与点单场景分别计算，综合当前意图、长期偏好和价格接近度后按推荐分排序
- 首次加购和支付成功后事务提交异步刷新；每 10 分钟增量刷新浏览等变化，每天 03:00 全量校准
...
6. **AI图片处理** - 批量异步处理商品图片
7. **微信订阅消息** - 取餐通知推送
8. **可解释用户画像推荐** - 浏览、加购、购买分级加权，结合当前意图、标签偏好和价格接近度排序
