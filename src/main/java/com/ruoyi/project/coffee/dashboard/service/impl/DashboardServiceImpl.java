...
public class DashboardServiceImpl implements IDashboardService
{
    ...
    private static final int HIGH_URGE_THRESHOLD = 2;
    ...
    @Override
    public DashboardOverviewVO getOverview(String businessType, String period, String startDate, String endDate)
    {
        // 默认为扫码点单
        String normalizedType = StringUtils.isEmpty(businessType) ? "scan" : businessType.trim().toLowerCase();

        PeriodWindow window = buildPeriodWindow(period, startDate, endDate);
        DashboardQueryDTO query = window.getQuery();
        query.setBusinessType(normalizedType);
        ...
        {
            // 扫码点单数据
            overview.setSalesTrend(fillAmountTrend(dashboardMapper.selectScanSalesTrend(query), window.getStartDate(), window.getEndDate()));
            overview.setOrderStatusDistribution(buildScanStatusDistribution(dashboardMapper.selectScanOrderStatusDistribution(query)));
            overview.setTopProducts(buildTopProducts(dashboardMapper.selectScanTopProducts(query)));
            overview.setCategorySalesShare(buildCategoryShare(dashboardMapper.selectScanCategorySalesShare(query)));
        }
        else
        {
            // 商城数据
    ...
    private AlertSummaryVO buildAlerts(DashboardQueryDTO query)
    {
        ...
        if ("scan".equals(query.getBusinessType()))
        {
            // 扫码点单告警
            alerts.setPendingAcceptCount(defaultLong(dashboardMapper.countScanPendingAccept()));
            alerts.setHighUrgeCount(defaultLong(dashboardMapper.countScanHighUrge(query)));
    ...
    private KpiSummaryVO buildKpis(DashboardQueryDTO query)
    {
        ...
        {
            // 扫码点单KPI
            BigDecimal salesAmount = money(dashboardMapper.sumScanSalesAmount(query));
            Long paidOrderCount = defaultLong(dashboardMapper.countScanPaidOrders(query));
            kpis.setSalesAmount(salesAmount);
            kpis.setOrderCount(defaultLong(dashboardMapper.countScanOrders(query)));
            ...
            kpis.setPendingAcceptCount(defaultLong(dashboardMapper.countScanPendingAccept()));
            kpis.setMakingCount(defaultLong(dashboardMapper.countScanMaking()));
            kpis.setAverageWaitMinutes(money(dashboardMapper.avgScanWaitMinutes(query)));
            kpis.setUrgeCount(defaultLong(dashboardMapper.sumScanUrgeCount(query)));
        }
        else
        {
            // 商城KPI
    ...
    private List<PieItemVO> buildScanStatusDistribution(List<StatusCountVO> rows)
    {
        Map<Integer, Long> countMap = new HashMap<Integer, Long>();
        if (rows != null)
        {
        ...
        for (int status = 0; status <= 5; status++)
        {
            PieItemVO item = new PieItemVO();
            Long count = countMap.containsKey(status) ? countMap.get(status) : 0L;
            item.setName(scanOrderStatusLabel(status));
            item.setCount(count);
            item.setValue(BigDecimal.valueOf(count));
            result.add(item);
        }
    ...
    private PeriodWindow buildPeriodWindow(String period, String startDate, String endDate)
    {
        ...
        query.setHighUrgeThreshold(HIGH_URGE_THRESHOLD);
    ...
    private String scanOrderStatusLabel(int status)
    {
        switch (status)
        {
            case 0:
